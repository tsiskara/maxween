...
/* ---------- Config (must match client constants exactly) ----------
   Values are read from env once at module load, then frozen. This keeps
   crashFromHash deterministic for any single deployment (changing the edge
   requires a redeploy, which is auditable — no runtime knob that an
   operator could flip mid-session to retroactively change settled math).
   The client fetches these via GET /api/config so its verify() stays
   consistent with the server that produced the commitment. */
...
const HOUSE_EDGE = numEnv('BOOST_HOUSE_EDGE', 0.01, 0.005, 0.05); // 0.5%–5%
const MAX_MULT = Math.floor(numEnv('BOOST_MAX_MULT', 1000, 2, 1e9));

// Table limits — the operator's underwriting cap. A single bet can never
// pay more than MAX_PAYOUT_USDT regardless of multiplier, and the stake
// itself is capped at MAX_STAKE_USDT. These are the levers that make the
// game insurable: without them a whale could theoretically win stake*1000.
const MAX_STAKE_USDT = numEnv('BOOST_MAX_STAKE_USDT', 1000, 0.01, 1e9);
const MAX_PAYOUT_USDT = numEnv('BOOST_MAX_PAYOUT_USDT', 50000, 1, 1e12);
...
/* Effective cashout multiplier after the payout cap is applied.
   Given a stake, the most we will ever pay is MAX_PAYOUT_USDT, so the
   effective ceiling on the multiplier is MAX_PAYOUT_USDT / stake. This is
   enforced at settlement (settle_win_by_token caps payout) AND surfaced
   to the client (so the UI shows the real ceiling). Pure function — same
   on client and server. */
export function effectiveMaxMult(stake) {
  if (!Number.isFinite(stake) || stake <= 0) return CFG.MAX_MULT;
  return Math.min(CFG.MAX_MULT, CFG.MAX_PAYOUT_USDT / stake);
}
...
/* ---------- Operator controls (legitimate, disclosed) ----------
   ...
   - MAINTENANCE: set BOOST_MAINTENANCE=1 to return 503 on money endpoints.
     Players in flight still settle (their tokens are valid); new bets are
     refused. Rotating BOOST_SECRET additionally invalidates in-flight round
     tokens for a hard pause.
